...
export class MySqlContainer extends GenericContainer {
  ...
  constructor(image = 'mariadb:latest') {
    super(image)
  }
  ...
  public override async start(): Promise<StartedMySqlContainer> {
    console.log(...(this.hasExposedPorts ? this.exposedPorts : [MYSQL_PORT]))
    this.withExposedPorts(
      ...(this.hasExposedPorts ? this.exposedPorts : [MYSQL_PORT])
    )
      .withEnvironment({
...
export class StartedMySqlContainer extends AbstractStartedContainer {
  ...
  public async executeQuery(
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `executeQuery failed with exit code ${result.exitCode} for query: ${query}`
      )
    }
